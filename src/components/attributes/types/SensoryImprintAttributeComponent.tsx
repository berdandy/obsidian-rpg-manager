...
function EditComponent({
  ...
  const [sensoryImprint, setSensoryImprint] =
    React.useState<SensoryImprintInterface>(
      attribute.value as SensoryImprintInterface,
    );

  const [sight, setSight] = React.useState<string>(
    attribute?.value?.sight ?? "",
  );
  const [hear, setHear] = React.useState<string>(attribute?.value?.hear ?? "");
  const [smell, setSmell] = React.useState<string>(
    attribute?.value?.smell ?? "",
  );
  const [touch, setTouch] = React.useState<string>(
    attribute?.value?.touch ?? "",
  );
  const [taste, setTaste] = React.useState<string>(
    attribute?.value?.taste ?? "",
  );
  ...
  const handleGeneration = () => {
    ...
    if (element.attribute(AttributeType.Description).isSet)
      chatGpt.description = element.attribute(AttributeType.Description)
        .value as string;

    if (element.attribute(AttributeType.Behaviour).isSet)
      chatGpt.description = element.attribute(AttributeType.Behaviour)
        .value as string;
  ...
  const handleSave = () => {
    ...
    codeblockService
      .updateCodeblockData(attribute.id, sensoryImprint)
      .then(() => {
        onSave();
      });
  };
...
function BrowseComponent({
  ...
  const { t } = useTranslation();
  const sensoryImprint: SensoryImprintInterface =
    attribute.value as SensoryImprintInterface;
