...
const initialStepComponents: StepComponentInterface[] = [
  ...
  {
    name: "Description",
    component: NonPlayerCharacterWizardDescriptionStepComponent,
  },
];
...
const stepComponents: StepComponentInterface[] = [
  ...
  {
    name: "Occupation",
    component: NonPlayerCharacterWizardOccupationStepComponent,
  },
  {
    name: "Character Arc",
    component: NonPlayerCharacterWizardArcStepComponent,
  },
  {
    name: "Beliefs",
    component: NonPlayerCharacterWizardBeliefsStepComponent,
    chatGptId: "beliefs",
  },
  {
    name: "Ghost",
    component: NonPlayerCharacterWizardGhostStepComponent,
    chatGptId: "ghost",
  },
  {
    name: "Lie",
    component: NonPlayerCharacterWizardLieStepComponent,
    chatGptId: "lie",
  },
  {
    name: "Need",
    component: NonPlayerCharacterWizardNeedStepComponent,
    chatGptId: "need",
  },
  {
  ...
  {
    name: "Behaviour",
    component: NonPlayerCharacterWizardBehaviourStepComponent,
    chatGptId: "behaviour",
  },
  {
    name: "Want",
    component: NonPlayerCharacterWizardWantStepComponent,
    chatGptId: "want",
  },
  { name: "Stake", component: NonPlayerCharacterWizardStakeStepComponent },
  {
    name: "Opposition",
    component: NonPlayerCharacterWizardOppositionStepComponent,
    chatGptId: "opposition",
  },
];
...
export default function NonPlayerCharacterWizardComponent({
  ...
  const [stepsType, setStepsType] = React.useState<StepComponentInterface[]>(
    initialStepComponents,
  );
  ...
  const chatGpt = React.useRef<ChatGptNonPlayerCharacterModel | undefined>(
    undefined,
  );
  ...
  const createAutomatically = async () => {
    ...
    for (let index = 0; index < stepsType.length; index++) {
      ...
      if (
        step.chatGptId !== undefined &&
        wizardData[step.chatGptId] === undefined
      ) {
          ...
          case "sw":
            ...
            if (responses.length > 0) {
              ...
              for (const strength of responses) {
                const normalizedStrength =
                  ...
                  strength.slice(1).toLowerCase();

                if (
                  StrengthType[
                    normalizedStrength as keyof typeof StrengthType
                  ] !== undefined
                )
                  strengthValue |=
                    StrengthType[
            ...
            if (responses.length > 0) {
              ...
              for (const weakness of responses) {
                ...
                if (
                  WeaknessType[
                    normalizedWeakness as keyof typeof WeaknessType
                  ] !== undefined
                )
                  weaknessValue |=
                    WeaknessType[
  ...
  const setCurrentChatGPT = () => {
    ...
    if (wizardData.occupation !== undefined)
      chatGpt.current.occupation = wizardData.occupation;
    if (wizardData.arc !== undefined)
      chatGpt.current.characterArc = wizardData.arc as ArcType;
    if (wizardData.beliefs !== undefined)
      chatGpt.current.beliefs = wizardData.beliefs;
    if (wizardData.ghost !== undefined)
    ...
    if (wizardData.behaviour !== undefined)
      chatGpt.current.behaviour = wizardData.behaviour;
    if (wizardData.want !== undefined) chatGpt.current.want = wizardData.want;
    if (wizardData.opposition !== undefined)
      chatGpt.current.opposition = wizardData.opposition;
  };

  if (Object.keys(wizardData).length === 0) {
    wizardData.nonplayercharactertype = element?.attribute(
      AttributeType.NonPlayerCharacterType,
    )?.value;
    wizardData.description = element?.attribute(
